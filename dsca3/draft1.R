...
# is determined at the point, beyond which the remaining eigenvalues are all relatively small and of comparable size 
...
n2 <- subset(df,select = c("Date","temp"))
n2
str(n2)
colSums(is.na(n2))
class(n2)
...
str(n2$temp)
plot(dsd)
cd <- decompose(dsd, type = c("multiplicative"), filter = NULL)
plot(cd)
which(is.na(dsd))
colnames(df)
